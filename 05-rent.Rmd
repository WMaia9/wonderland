...
After all, from an early age, we are led to believe that homeownership is a goal that everyone should have and strive to achieve. This, however, is not an absolute truth.

Furthermore, having a home, for many, is synonymous with stability and security. But, is it worth buying a house? Or can continue to rent make more sense?
...
Initially, we will see how the literature says what the amount should be charged in rent.
...
But many variables make up the rent calculation. The real estate market itself regulates itself and prices the rental price of properties by supply and demand. Supply and demand are important concepts of the Liberal Economy.
...
The first application can be someone who wants to rent a house and is looking for more attractive values. With the model, it is possible to search for regions of interest and find out if the rental amount that the owner is charging is worth it or not. In addition, it is possible to search for properties by price range.
...
For future work, we can make a web scraper again on the site, take images of the properties and apply a convolutional neural network, aggression that could help in the accuracy of the model with real photos inside the properties.
